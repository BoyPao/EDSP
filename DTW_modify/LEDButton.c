...
section("L1_data_b") fract16 fft_mag[100];			// output magnitude
section("L1_data_b") fract16 fft_in[VECTOR_SIZE];			// input
//section("L1_data_b") complex_fract16 t[VECTOR_SIZE];
section("L1_data_b") fract16 Record[7400];
section("L1_data_a") short frame_count = 0;
section("L1_data_a") short energy_count=0;
...
section("L1_data_a") int which = 0;
//section("L1_data_a") short choose;
section("L2_shared") fract16 test[2220]; 
...
section("L2_shared") fract16 zero[2220]; 
section("L2_shared") fract16 one[2220];  
section("L2_shared") fract16 two[2220];  
section("L2_shared") fract16 three[2220];
section("L2_shared") fract16 DTW[74][74]={0};
section("L2_shared") fract16 c[74][74]={0};
//section("L2_shared") short DTW_2[45][45]={0};
section("L1_data_a") short energy=0;
section("L1_data_a") short threshold=0;
section("L1_data_a") short threshold1=0;
...
void main(void) {
	...
	while(1)
	{
     ...
     while(1)
     {
        if (ezIsButtonPushed(EZ_FIRST_BUTTON+1))	//train number 'two'
         	{      
         		ezDelay(100);      						
                ezClearButton(EZ_FIRST_BUTTON+1);	//clear the button
                ezTurnOffAllLEDs();
      			train_two();						//call the function of training
     ...
     while(1)
     {
         if (ezIsButtonPushed(EZ_FIRST_BUTTON+2))	//train number 'three'
         	{      
         		ezDelay(100);      
                ezClearButton(EZ_FIRST_BUTTON+2);	//clear the button
                ezTurnOffAllLEDs();
                train_three();						//call the function of training
	...
	while(1)
     {
         if (ezIsButtonPushed(EZ_FIRST_BUTTON+4))
         	{      
         		ezDelay(100);      
                ezClearButton(EZ_FIRST_BUTTON+4);
                ezTurnOffAllLEDs();
                which=compute_dtw();
